...
// end structs
    ...
    function voteToAddMember(address newMember) external memberOnly {
        ...
        require(approvalVotes[newMember].votesCastByMembers[msg.sender] == 0, "you have already voted to add this user");
        // cast the votes
        ...
        if (approvalVotes[newMember].votes > totalVotes/2 && userVotes[newMember] == 0) {
    ...
    function createEvent(string memory title, string memory description) external memberOnly returns (ClubEvent memory) {
    ...
    function proposeSpendClubETH(address recipient, uint256 amount, string memory description) external memberOnly {
        uint256 id = paymentsCount;

        ClubPayment memory newPayment = ClubPayment(id, recipient, amount, description, userVotes[msg.sender], false);
    ...
    function voteSpendClubETH(uint256 paymentId) external memberOnly {
        require(clubPayments[paymentId].votes > 0, "payment id does not exist!");
        require(votesCastByMembersForPayment[paymentId][msg.sender] == 0, "you have already voted!");
        require(!clubPayments[paymentId].accepted, "payment has already been accepted!");
        clubPayments[paymentId].votes += userVotes[msg.sender];
        votesCastByMembersForPayment[paymentId][msg.sender] = userVotes[msg.sender];
    ...
    function viewSpendETHProposals() public view returns (ClubPayment[] memory) {
        return clubPayments;
    }
